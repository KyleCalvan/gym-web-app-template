...
export function Ticker({ to, duration = dur.ticker, prefix = '', suffix = '', format, asText }) {
  ...
  if (asText) {
    return <motion.text ref={nodeRef}>{initial}</motion.text>;
  }
  return <motion.span ref={nodeRef}>{initial}</motion.span>;
}
...
export { motion, useMotionValue, useTransform, animate, useInView, useAnimationControls, useEffect };